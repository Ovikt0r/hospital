...
<div>
    <div class="btn-group">
        ...
        <ul class="dropdown-menu">
            <li><a class="dropdown-item"
                   href="controller?action=${param.action}&sorting=asc&sortBy=name${requestScope.categoryId != null ? requestScope.categoryId : ''}">
                <fmt:message key="paged.sort.name.asc"/>
            </a></li>
            <li><a class="dropdown-item"
                   href="controller?action=${param.action}&sorting=desc&sortBy=name${requestScope.categoryId != null ? requestScope.categoryId : ''}">
                <fmt:message key="paged.sort.name.desc"/>
            </a></li>
            <li><a class="dropdown-item"
                   href="controller?action=${param.action}&sorting=asc&sortBy=num${requestScope.categoryId != null ? requestScope.categoryId : ''}">
                <fmt:message key="paged.doctors.sort.num.asc"/>
            </a></li>
            <li><a class="dropdown-item"
                   href="controller?action=${param.action}&sorting=desc&sortBy=num${requestScope.categoryId != null ? requestScope.categoryId : ''}">
    ...
    <table class="table table-hover table-striped">
        ...
        <c:forEach begin="0" end="${requestScope.pagedDoctors.doctors().size()}" varStatus="loop"
            ...
            <tr onclick="location.href='controller?action=${param.action}&doctorId=${item.id}${param.dataToSave}'">
        ...
        <ul class="pagination">
            <c:forEach begin="1" end="${requestScope.pagedDoctors.numOfPages()}" varStatus="loop">
                <li class="page-item ${requestScope.pagedDoctors.pageNum() == loop.index ? "active" : ""}">
                    <a class="page-link"
                       href="controller?action=${param.page_action}&sorting=${requestScope.doctors_sorting}&sortBy=${requestScope.doctors_sort_by}&pageNum=${loop.index}">
                            ${loop.index}
                    </a>
                </li>
            </c:forEach>
