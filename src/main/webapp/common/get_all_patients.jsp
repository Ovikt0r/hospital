...
<div>
    <div class="btn-group">
        ...
        <ul class="dropdown-menu">
            <li><a class="dropdown-item" href="controller?action=patients&sorting=asc&sortBy=name">
                <fmt:message key="paged.sort.name.asc"/>
            </a></li>
            <li><a class="dropdown-item" href="controller?action=patients&sorting=desc&sortBy=name">
                <fmt:message key="paged.sort.name.desc"/>
            </a></li>
            <li><a class="dropdown-item" href="controller?action=patients&sorting=asc&sortBy=birthday">
                <fmt:message key="paged.patients.sort.birthday.asc"/>
            </a></li>
            <li><a class="dropdown-item" href="controller?action=patients&sorting=desc&sortBy=birthday">
        ...
        <ul class="pagination">
            <c:forEach begin="1" end="${requestScope.pagedPatients.numOfPages()}" varStatus="loop">
                <li class="page-item ${requestScope.pagedPatients.pageNum() == loop.index ? "active" : ""}">
                    <a class="page-link"
                       href="controller?action=patients&sorting=${requestScope.patientsSorting}&sortBy=${requestScope.patientsSortBy}&pageNum=${loop.index}">
                            ${loop.index}
                    </a>
                </li>
            </c:forEach>
